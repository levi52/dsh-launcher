...
- 📲 **可安装为应用**：PWA 支持（manifest + Service Worker），浏览器地址栏「安装」后以独立窗口运行，带鲸鱼图标
...
**发现新版本？** 更新检查发现新版本后，「安装」按钮变为 **「升级到 vX」**，一键执行全局升级，同样实时输出。

**想卸载 dsh？** 设置页「已发现的 dsh 安装」区有 **「卸载全局 dsh…」按钮**（存在全局安装时显示，带明确警告与确认；运行中禁止卸载）。也可手动执行：
