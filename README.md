...
| **设置** | Web UI 端口与主机、工作区、dsh CLI 入口、DSH_HOME 覆盖、自动打开浏览器 |
...
- **dsh CLI 探测**：优先使用配置的 `dshCommand`；否则依次查找 npx 缓存（`%LOCALAPPDATA%\npm-cache\_npx\*\node_modules\@deepseek-ai\dsh\lib\bin.js`）、全局 npm 安装；最后兜底 `npx -y @deepseek-ai/dsh`（需联网）。解析到本地 `bin.js` 时用当前 Node 解释器执行。
- **端口占用**：点击启动时若目标端口已被占用且不是启动器拉起的进程，提示「Web UI 似乎已在运行」，直接打开界面即可，不会抢占端口。
- **停止语义**：「停止服务」只对启动器自己拉起的进程有效（安全设计，避免误杀）；需要关闭其他实例时使用「强制停止端口进程」。
